...
KUBERNETES_VERSION="${KUBERNETES_MINOR}.2"
CRUN_VERSION='1.9'
# OCI_RUNTIME='crun'
OCI_RUNTIME='runc'
# CRI='containerd'
...
_remove_unneeded_packages () {
  ...
  apt-get -qy purge multipath-tools packagekit polkitd snapd
...
_add_k8s_repositories () {
  ...
  if [ "$CRI" = "crio" ] || [ "$CRI" = "cri-o" ]; then
    curl -fsSL https://pkgs.k8s.io/addons:/cri-o:/stable:/v$KUBERNETES_MINOR/deb/Release.key | gpg --dearmor -o /etc/apt/keyrings/cri-o-apt-keyring.gpg
    echo "deb [signed-by=/etc/apt/keyrings/cri-o-apt-keyring.gpg] https://pkgs.k8s.io/addons:/cri-o:/stable:/v$KUBERNETES_MINOR/deb/ /" >/etc/apt/sources.list.d/cri-o.list
  fi
  apt-get -q update
}
